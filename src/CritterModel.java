...
public class CritterModel {
    ...
    private Info getInfo(PrivateData data, Class original) {
        Critter.Neighbour[] neighbours = new Critter.Neighbour[4];
        Critter.Direction d = data.direction;
        boolean[] neighborThreats = new boolean[4];
        for (int i = 0; i < 4; i++) {
            neighbours[i] = getStatus(pointAt(data.p, d), original);
            if (neighbours[i] == Critter.Neighbour.OTHER) {
        ...
        return new Info(neighbours, data.direction, neighborThreats);
    }

    private Critter.Neighbour getStatus(Point p, Class original) {
        if (!inBounds(p))
            return Critter.Neighbour.WALL;
        else if (grid[p.x][p.y] == null)
            return Critter.Neighbour.EMPTY;
        else if (grid[p.x][p.y].getClass() == original)
            return Critter.Neighbour.SAME;
        else
            return Critter.Neighbour.OTHER;
    }
    ...
    private static class Info implements CritterInfo {
        private Critter.Neighbour[] neighbours;
        private Critter.Direction direction;
        private boolean[] neighborThreats;

        public Info(Critter.Neighbour[] neighbours, Critter.Direction d,
